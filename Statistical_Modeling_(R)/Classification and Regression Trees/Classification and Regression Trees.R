...
mower.df$Ownership <- factor(mower.df$Ownership, levels = c("Owner", "Nonowner"))
...
# Partition into Training (60%) and Validation (40%) sets
set.seed(1)
...
# Build the default (best-pruned) classification tree
...
# Each row represents a different level of the tree
...
# The first xerror that drops below the minimum line
...
prp(toyota.default.rt, type = 1, extra = 1, under = TRUE, split.font = 2,
    under.font = 1, nn.font = 3,varlen = -10,
    box.col = ifelse(toyota.default.rt$frame$var == "<leaf>", "gray", "white"))