...
# Table 7.2
# Normalize variables using mean and standard deviation extracted from traing set only
...
for(i in 1:dim(accuracy.df)[1]){
  ...
  accuracy.df$accuracy[i] <- confusionMatrix(knn.pred.valid,
                                             norm.valid.df[, 3],
                                             positive = "Nonowner")$overall[1]
}
# Print the results
accuracy.df

# Table 7.4
# Run kNN with the best k value
knn.pred.new <- knn(norm.mower.df[, 1:2], norm.new.df, cl = norm.mower.df[, 3], k = 5)
# Show the identified 8 nearest neighbors of the new record
